 ...
 * Uses a module pattern, state management (load/error), and best practices.
 */
...
$(function () {
    ...
    const orderDetailsPage = {
        ...
        config: {
           ...
           actionConfig: {
                // Main Actions (buttons always visible if active)
        ...
        //================================================================================
            ...
            if (window.location.hash) {
                ...
                const idFromHash = hs.get("order_number") || hs.get("order_id") || hs.get("orderNumber");
                if (idFromHash) return idFromHash;
            }

            // 2) Query (?order_number=... / ?order_id=...)
            const qs = new URLSearchParams(window.location.search);
            const idFromQuery = qs.get("order_number") || qs.get("order_id") || qs.get("orderNumber");
            if (idFromQuery) return idFromQuery;

            // 3) Ruta tipo /orderDetail/ORD12345
            const m = window.location.pathname.match(/orderDetail(?:\.html)?\/([^\/?#]+)/i);
        ...
        generateActionsHTML: function(order) {
            ...
            for (const actionKey in this.config.actionConfig) {
                ...
                // We check whether the action is visible for the current state.
            ...
            // If there are actions in the drop-down menu, we create it.
        ...
        //================================================================================
        // 7. LOGIC OF THE EDIT RECIPIENT DATA MODE
        //================================================================================
        ...
        initEditItemsModal: function() {
            ...
            // event to open the edit items modal
            $(document).off('click', '#editItemsButton').on('click', '#editItemsButton', () => {
                if (!self.currentOrderData) return;

                const status = self.currentOrderData.status;

                // condition only edit items if the status is between 1 and 9
                // (inclusive), otherwise show the 'not allowed' modal
                if (status >= 1 && status <= 9) {
            ...
            // Event listeners for the edit items modal
        ...
        buildMockProductDatabase: function(allOrders) {
           const allItems = allOrders.flatMap(order => order.items || []);
            ...
            allItems.forEach(item => {
                ...
                // Only add unique items based on item_name
                if (itemName && !uniqueItems.has(itemName)) {
                    uniqueItems.set(itemName, {
                        name: itemName,
                        sku: item.sku || 'N/A', // save SKU if available
