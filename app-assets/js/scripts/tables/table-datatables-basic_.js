    ...
    if (dt_basic_table.length) {
        const dt_basic = dt_basic_table.DataTable({
            ...
            columnDefs: [
                ...
                {
                    targets: -1, title: 'Actions', className: 'no-modal actions-column', orderable: false,
                    render: function (data, type, full, meta) {
                        return `<div class="d-flex gap-1">
                                    <a href="orderDetail.html?order_number=${full.order_number}" class="btn px-25 py-25 btn-sm btn-outline-primary m-0 ">${feather.icons['eye'].toSvg({ class: 'me-25' })} Details</a>
                                    <a href="https://wemalo.example.com/order/${full.external_order_id}" target="_blank" class="btn btn-outline-primary btn-sm px-25 py-25 m-0">${feather.icons['external-link'].toSvg({ class: 'me-25' })} wemalo</a>
                                </div>`;
                    }
                }
            ],
            ...
            responsive: {
                details: {
                    display: $.fn.dataTable.Responsive.display.childRow, type: 'column', target: 'tr',
        ...
        $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
            ...
            if (activeStatusGroup && !STATUS_GROUPS[activeStatusGroup].statuses.includes(rowData.status)) return false;
        ...
        function renderDrillDownCards(groupName) {
            ...
            groupStatuses.forEach(status => {
                ...
                if (count > 0) {
                    ...
                    $container.append(`<div class="card status-card-small bg-light shadow-none border mb-0" data-status="${status}" style="min-width: 180px;"><div class="card-body d-flex p-1"><div class="avatar bg-light-${info.class.replace('bg-', '')} me-1"><div class="avatar-content"><i data-feather="${info.icon}" class="font-medium-2"></i></div></div><div class="d-flex flex-column"><h6 class="fw-bolder mb-0">${count}</h6><small>${info.title}</small></div></div></div>`);
                }
            });
            $section.show();
            feather.replace();
