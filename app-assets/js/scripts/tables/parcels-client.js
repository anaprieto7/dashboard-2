...
$(function () {
    ...
    if (parcelsTableElement.length) {
        ...
        // Delegado en el tbody para que funcione con redraws/responsive
            $('#parcels-table tbody')
            .off('click.parcelsGo', 'a.js-go-order')
            .on('click.parcelsGo', 'a.js-go-order', function (e) {
                e.preventDefault();
                e.stopPropagation();

                const id = $(this).data('order');

                // Ruta relativa (asumiendo que orderDetailClient.html está en la misma carpeta)
                // Pasamos el ID en query y también en hash (backup contra rewrites)
    ...
    function updateBadges() {
        ...
        if (dateRange.trim()) activeFilters.append(createBadge('date', dateRange, `Date: ${dateRange}`));
